...
class HybridSearch:
    ...
    def weighted_search(self, query: str, alpha, limit: int= 5):
        bm25results = self._bm25search(query, limit)
        bm25_scores = [x["score"] for x in bm25results]
        bm25_norms = normalize_score(bm25_scores)

        semresults = self.semantic_search.chunk_search(query, limit)
        ...
        for i, result in enumerate(semresults):
            ...
            if doc_id not in combined:
                combined[doc_id] = {"title": result["title"],
                "bm25score": 0.0,
                "semscore": sem_norms[i],
                "hybridscore": 0.0
                }
            else:
